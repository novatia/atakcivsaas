...
# OpenTAKServer a partire da questo repo.
#
# Il plugin è il fork in ots/plugins/OTS-SkyFi-Plugin (l'upstream
# brian7704/OTS-SkyFi-Plugin non distribuisce la UI): la UI è statica e
# inclusa nel pacchetto, quindi non servono node/npm.
#
# Uso (da root sul server, dentro il clone del repo):
#   ./update-skyfi-plugin.sh            # (ri)installa il plugin + restart + verifica
#   ./update-skyfi-plugin.sh --check    # mostra solo la versione installata
#   ./update-skyfi-plugin.sh --pull     # prima fa git pull del repo, poi installa
#
...
if [[ -n "${CURRENT}" ]]; then
    ...
    # La 0.0.0 è l'upstream installato da GitHub, senza UI: viene rimpiazzata
    [[ "${CURRENT}" == "0.0.0" ]] && log "(0.0.0 = versione upstream senza UI: verrà sostituita dal fork del repo)"
else
...
# La UI statica deve essere finita nel pacchetto installato
if ! "${PIP}" show -f "${PLUGIN_DISTRO}" 2>/dev/null | grep -q 'ui/index.html'; then
...
step "Fatto"
log "UI del plugin (solo utenti administrator): https://<server>/api/plugins/ots_skyfi_plugin/ui"
log "API key SkyFi: tab Configurazione della UI (o OTS_SKYFI_PLUGIN_API_KEY in config.yml)."
