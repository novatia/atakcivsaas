        ...
        try:
            for key, value in config.items():
                if key not in DefaultConfig.__dict__.keys():
                    return {"success": False, "error": f"{key} non è una chiave di configurazione valida"}
                if key == "OTS_SKYFI_PLUGIN_API_KEY" and not isinstance(value, str):
                    return {"success": False, "error": f"{key} deve essere una stringa"}
                if key == "OTS_SKYFI_PLUGIN_ENABLED" and not isinstance(value, bool):
                    return {"success": False, "error": f"{key} deve essere un booleano"}
